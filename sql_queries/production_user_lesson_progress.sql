/*
production_user_lesson_progress.sql

Grain: one row per user-lesson combination.

This is the "melted" version of production_user_subject_progress.sql.
Each row represents a single allocated lesson for a user, with its
completion status, score, rating, and cumulative duration.

Use this when you need:
  - Lesson-level analysis without JSON parsing
  - Direct filtering or aggregation by lesson type, assessment flag, etc.
  - A tabular feed to BI tools that don't support JSON columns

To derive subject-level summaries: GROUP BY user_id, subject_id and
aggregate completed, score, duration etc.
...
ple_allocation AS (
    ...
    WHERE u.user_type IN (3, 4)
      ...
      -- Career-path mapping is required for EVERY subject, is_ple = 0 included.
      -- The former `s.is_ple = 0 OR ...` bypass allocated subjects the app never
      -- shows the learner; those rows are permanently uncompletable and deflate completion %.
...
-- ── FINAL SELECT: one row per user-lesson (flat, no JSON) ─────────────────────
-- Each row is one allocated lesson for a user.
-- completed = 1 means the user has at least one completed activity for this lesson.
-- score / rating / duration are NULL when completed = 0.
SELECT
    m.user_id,
    au.organisation_id,
    m.user_type,

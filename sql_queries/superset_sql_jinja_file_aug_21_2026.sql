-- ARCHIVED SNAPSHOT — Superset virtual dataset as it stood on 2026-08-21.
-- Kept for reference only. The live version is superset_sql_jinja_file.sql.
--
-- Differences from the current file:
--   * reads production_users_one_record_new (the staging rebuild) rather than the
--     live table
--   * the programme filter in the JOIN is positional:
--       JSON_EXTRACT(project_combos, '$[0].prog_name') IN (...)
--     which inspects only the first array entry. Harmless today — all 885,098
--     users pass either way — but it breaks as soon as a user's first combo
--     belongs to another programme.
--
-- Do not paste this into Superset; use superset_sql_jinja_file.sql.
...
-- JOIN quest_analytics.user_addon_1 b ON
	quest_analytics.production_users_one_record_new a
